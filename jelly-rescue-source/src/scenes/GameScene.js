...
export default class GameScene extends Phaser.Scene {
  ...
  createPlayers() {
    const key1 = this.input.keyboard.addKeys({ left: 'A', right: 'D', jump: 'W' });
    const cursors = this.input.keyboard.createCursorKeys();
    const key2 = { left: cursors.left, right: cursors.right, jump: cursors.up };
  ...
  updateTimer() {
    ...
    if (seconds === 0) {
      ...
      this.time.delayedCall(1100, () => this.scene.restart({ levelIndex: this.levelIndex }));
    }
  }
